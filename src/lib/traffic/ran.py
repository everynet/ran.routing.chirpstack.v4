...
class UplinkAckSync:
    def __init__(self, ttl=60) -> None:
        self._cache: Cache[bytes, UplinkAck] = Cache(ttl=ttl)
        self._waiters: dict[bytes, set[asyncio.Future[UplinkAck]]] = defaultdict(set)
        self._lock = asyncio.Lock()

    def set_ack(self, context_id: bytes, uplink_ack: UplinkAck):
        self._cache.set(context_id, uplink_ack)

    def has_ack(self, context_id: bytes) -> bool:
        return self._cache.get(context_id, None) is not None

    def get_ack(self, context_id: bytes) -> UplinkAck | None:
    ...
    async def make_ack_waiter(self, context_id: bytes) -> asyncio.Future[UplinkAck]:
...
class RanTrafficRouter:
    def __init__(self, ran_core: RANCore) -> None:
        ...
        # Actually, we don't need to store "upstream_id" as value in this cache, because we only use it to check
        #   entries for deduplication. upstream_id value used only for logging purposes at now.
        self._context_id_to_upstream_id: Cache[bytes, int] = Cache(ttl=60)
        self._upstream_id_to_transaction_id: Cache[int, int] = Cache(ttl=60)

        # Cache for tracking which downstream must be answered with DownstreamResult
        self._downstream_transaction_to_id: Cache[int, int] = Cache(ttl=60)
        ...
        # Mic challenge state
        self._mic_tries_count: dict[int, int] = {}
    ...
    async def handle_upstream_ack_or_reject(self, ack_or_reject: UplinkAck | UplinkReject) -> None:
        ...
        transaction_id = self._upstream_id_to_transaction_id.get(ack_or_reject.uplink_id, None)
    ...
    async def _handle_upstream_message(self, upstream_message: UpstreamMessage):
        # Upstream hash will be used as "context_id" for uplink message
        context_id = hashlib.md5(bytes(upstream_message.phy_payload_no_mic), usedforsecurity=True).digest()
        ...
        if (upstream_id := self._context_id_to_upstream_id.get(context_id)) is not None:
            logger.debug(
                "Upstream message is duplicate of message, already answered with downlink. "
                "It will be not forwarded to chirpstack",
                current_transaction_id=upstream_message.transaction_id,
                duplicates_transaction_id=self._upstream_id_to_transaction_id.get(upstream_id, None),
                duplicates_upstream_id=upstream_id,
            )
            last_ack: UplinkAck | None = self._ack_sync.get_ack(context_id)
            ...
            uplink_ack_waiter = await self._ack_sync.make_ack_waiter(context_id)
            await self._ack_submitters_pool.add(
                self._submit_ack_when_available(upstream_message.transaction_id, uplink_ack_waiter)
            )
            return

        upstream_id = self._generate_upstream_id()
        self._context_id_to_upstream_id.set(context_id, upstream_id)
        self._upstream_id_to_transaction_id.set(upstream_id, upstream_message.transaction_id)
        total_challenges = len(upstream_message.mic_challenge)
        self._mic_tries_count[upstream_id] = total_challenges

        for mic, lora_message_bytes in self._populate_lora_messages(upstream_message):
            uplink = Uplink(
                uplink_id=upstream_id,
